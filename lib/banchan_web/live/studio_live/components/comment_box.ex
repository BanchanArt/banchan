defmodule BanchanWeb.StudioLive.Components.Commissions.CommentBox do
  ...
  def mount(socket) do
    {:ok, assign(socket, changeset: Commissions.change_event(%Event{
      type: :comment
    }, %{}))}
  end
  ...
  def handle_event("add_comment", %{"event" => event}, socket) do
    case Commissions.create_event(:comment, socket.assigns.actor, socket.assigns.commission, event) do
      {:ok, event} ->
        BanchanWeb.Endpoint.broadcast!("commission:#{socket.assigns.commission.public_id}", "new_comment", event)
        {:noreply, assign(socket, changeset: Commissions.change_event(%Event{
      type: :comment
    }, %{}))}
